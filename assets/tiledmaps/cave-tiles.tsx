<?xml version="1.0" encoding="UTF-8"?>
<tileset version="1.10" tiledversion="1.11.2" name="cave-tiles" tilewidth="32" tileheight="32" spacing="2" margin="1" tilecount="256" columns="16">
 <image source="tilesets/cave-tiles-extruded.png" width="544" height="544"/>
 <wangsets>
  <wangset name="Cave Walls" type="corner" tile="-1">
   <wangcolor name="Cave Wall" color="#00007f" tile="-1" probability="1"/>
   <wangtile tileid="0" wangid="0,0,0,1,0,0,0,0"/>
   <wangtile tileid="1" wangid="0,0,0,1,0,1,0,0"/>
   <wangtile tileid="2" wangid="0,0,0,0,0,1,0,0"/>
   <wangtile tileid="16" wangid="0,1,0,1,0,0,0,0"/>
   <wangtile tileid="17" wangid="0,1,0,1,0,1,0,1"/>
   <wangtile tileid="18" wangid="0,0,0,0,0,1,0,1"/>
   <wangtile tileid="19" wangid="0,1,0,0,0,1,0,1"/>
   <wangtile tileid="20" wangid="0,1,0,1,0,0,0,1"/>
   <wangtile tileid="32" wangid="0,1,0,0,0,0,0,0"/>
   <wangtile tileid="33" wangid="0,1,0,0,0,0,0,1"/>
   <wangtile tileid="34" wangid="0,0,0,0,0,0,0,1"/>
   <wangtile tileid="35" wangid="0,0,0,1,0,1,0,1"/>
   <wangtile tileid="36" wangid="0,1,0,1,0,1,0,0"/>
  </wangset>
 </wangsets>
</tileset>
