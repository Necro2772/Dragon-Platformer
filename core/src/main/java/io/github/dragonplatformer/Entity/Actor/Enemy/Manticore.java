...
public class Manticore extends Enemy {
    ...
    public Manticore(float x, float y, World world, AnimationManager animManager) {
        ...
        stats().setCrystalLoot((int) (Math.random() * 10) + 40);
    }
    ...
    public void act(float delta) {
        ...
        switch (getState()) {
            ...
            case FLYCHARGELUNGE:
                ...
                if (getStateTime() < targetTime && getStateTime() + delta > targetTime) target = new Vector2(stats().getPlayerPos());
                break;
            case LUNGE:
            ...
            case FLYSHOOTPROJECTILE:
                ...
                else if (anims.get(getState()).isAnimationFinished(getStateTime() + delta)) setState(attackPhase.removeFirst());
                break;
        }
...
//        }
    ...
    public void setState(EnemyState state) {
        switch (state) {
            case IDLE:
                if (!isGrounded()) setState(EnemyState.FLYIDLE);
            case CHARGELUNGE:
                if (!isGrounded()) setState(EnemyState.FLYCHARGELUNGE);
            case SHOOTPROJECTILE:
                if (!isGrounded()) setState(EnemyState.FLYSHOOTPROJECTILE);
            default:
