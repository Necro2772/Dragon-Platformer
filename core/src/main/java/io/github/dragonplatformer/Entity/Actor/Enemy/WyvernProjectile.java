...
public class WyvernProjectile extends Enemy {
    private final float idleWait;
    private final float attackWait;

    public WyvernProjectile(float x, float y, World world, AnimationManager animManager) {
    ...
    public void beginState() {
        ...
        switch (getState()) {
            ...
            case FLYSHOOTPROJECTILE:
                ...
                new Fireball(1, 5, 1, getBody().getPosition().x,
                    getBody().getPosition().y + getWidth() / 2f, 1, new Vector2(stats().getPlayerPos()),
                    animManager, false, getBody().getWorld());
        }
    }
}
